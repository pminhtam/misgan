...
def make_model(Dim,alpha = 10):
    ...
    MSE_train_loss = tf.reduce_mean((M * New_X - M * tf.math.round(G_sample)) ** 2) / tf.reduce_mean(M)

    D_loss = D_loss1
    G_loss = G_loss1 + alpha * MSE_train_loss

    # %% MSE Performance metric
    MSE_test_loss = tf.reduce_mean(((1 - M) * X - (1 - M) * tf.math.round(G_sample)) ** 2) / tf.reduce_mean(1 - M)
    MSE_loss_cols = tf.reduce_mean(((1 - M) * X - (1 - M) * tf.math.round(G_sample)) ** 2, axis=0) / tf.reduce_mean(1 - M, axis=0)
