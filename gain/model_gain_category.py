...
def make_model(Dim,alpha = 10):
    ...
    MSE_train_loss = tf.reduce_mean((M * (1- tf.cast(tf.equal(New_X , tf.math.round(G_sample)),tf.float32))) ** 2) / tf.reduce_mean(M)

    D_loss = D_loss1
    G_loss = G_loss1 + alpha * MSE_train_loss

    # %% MSE Performance metric
    MSE_test_loss = tf.reduce_mean(((1 - M) * (1- tf.cast(tf.equal(X ,tf.math.round(G_sample)),tf.float32))) ** 2) / tf.reduce_mean(1 - M)
    MSE_loss_cols = tf.reduce_mean(((1 - M) * (1-tf.cast(tf.equal(X ,tf.math.round(G_sample)),tf.float32))) ** 2, axis=0) / tf.reduce_mean(1 - M, axis=0)
...
def train(X, M, H, New_X, D_loss1, G_loss1, MSE_train_loss, MSE_test_loss, D_solver, G_solver, G_sample, Dim, Train_No,
          trainX, p_miss, sess, gain_iter, batch_size, p_hint,
          prob_masks, maskss=[0, 1]):
    # import tensorboardX
    # import torch
    p_miss = 1 / Dim
