...
def main():
    ...
    for mask in masks:
        ...
        with open("log_6col.txt","a+") as f:
            ...
            f.writelines( ".".join(list(mask)) + ":" + "|".join([str(re_all[k]) for k in [j for j in list(set(A) - set(mask))]])+"\n")
...
if __name__ == "__main__":
    ...
    # print(len([i for i in itertools.permutations(A,2)]))
    # print([i for i in itertools.combinations(A,2)])
    # maskes = [i for i in itertools.combinations(A,2)]
    # for mask in maskes:
    #     print(mask)
    #     print(A[k] for k in [j for j in list(set(A) - set(mask))])
    main()
