...
from gain.model_gain_category import *
import argparse
...
def main():
    ...
    # MSE_train_loss = tf.reduce_mean((M * tf.cast(tf.equal(New_X , tf.math.round(G_sample)),tf.float32)) ** 2) / tf.reduce_mean(M)
    G_sample = np.round(G_sample)
    # print("Re gen all  :",np.mean(np.square(np.subtract(G_sample,trainX)),axis=0))
    print("Re gen all  :",np.mean(1-(G_sample==trainX),axis=0))
    re_all = np.mean(1-(G_sample==trainX),axis=0)
